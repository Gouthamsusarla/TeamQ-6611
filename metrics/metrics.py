...
#Manages the transition from raw data generated by the DataGenerator to metric calculations performed by Metrics
class MetricsCalculator:
    ...
    def calculate_metric(self, metric):
        ...
        return getattr(self.metrics, metric_method)()
...
class MetricsGUI:
    ...
    def generate_data(self):
        try:
            ...
            data = self.controller.generate_data(n)
    ...
    def calculate_and_display(self, metric):
        try:
            if self.controller.metrics_calculator.metrics is not None and self.valid_input:
                # Convert metric label to snake_case
                metric_method = metric.lower().replace(" ", "_")
                result = self.controller.calculate_metric(metric)
                self.result_box.delete(1.0, tk.END)  # Clear existing results
...
#Manages the interaction between the GUI and the underlying logic
class Controller:
    def __init__(self, root):
        ...
        self.metrics_gui = MetricsGUI(root, self)

    def generate_data(self, n):
        data = self.data_generator.generate_data(n)
        self.metrics_calculator.set_data(data)
        self.generated_data.append(data)
        return data

    def calculate_metric(self, metric):
        return self.metrics_calculator.calculate_metric(metric)
